...
logger.add(f"../../result/{FLAGS.save_folder}/ddim_linf_lock/{FLAGS.model_type}/log_test_ddim_t_{Denoise_strength}_step_{Num_inference_steps}_|{model_id.split('/')[-1]}|_bs_{batch_size}_num_test_{num_test}_pgd_{pgd_step}_eot_{eot_step}.log")
...
class DDIMPipeline_Img2Img(DDIMPipeline):
    ...
    def __call__(
        ...
        for t in self.progress_bar(timesteps_list):
            ...
            # do x_t -> x_t-1
            variance_noise = reverse_noise[t]
            image = self.scheduler.step(
                model_output, t, image, eta=eta, use_clipped_model_output=use_clipped_model_output, generator=generator, variance_noise=variance_noise
            ).prev_sample
...
for sample_image, y_val in tqdm(cifar10_test_loader, colour='yellow'):
    ...
    reverse_noise = torch.randn([1000, batch_size, *sample_image.shape[1:]]).to(device)
    with torch.no_grad():
        if FLAGS.model_type == "vit":
    ...
    for _ in tqdm(range(pgd_step), colour='red'):
        ...
        for _ in range(1):
            ...
            noisy_image = noise_schdeuler.add_noise(sample_image + delta, noise_global, timesteps)

            images_1, images_2 = ddim(sample_image=noisy_image, batch_size=noisy_image.shape[0],
                                      timesteps_list=timesteps_list, reverse_noise=reverse_noise)
    ...
    with torch.no_grad():
        noise = torch.randn(sample_image.shape).to(device)
        noisy_image = noise_schdeuler.add_noise(adv_out, noise_global, timesteps)
        images_1, images_2 = ddim(sample_image=noisy_image, batch_size=noisy_image.shape[0], 
                                    timesteps_list=timesteps_list, reverse_noise=reverse_noise)

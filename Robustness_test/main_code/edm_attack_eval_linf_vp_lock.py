import os
from EDM import get_edm_cifar_uncond
import torch
import os
...
parser.add_argument("--ckpt", type=str, default='../../edm_cifar_uncond_vp.pt')
...
logger.add(f"../../edm_result_attack/{FLAGS.save_folder}/{dir_temp}/edm_vp_linf_attack/{FLAGS.model_type}/log_test_edm_{dir_temp}_random_strength_{FLAGS.random_strength}_sigma_max_{sigma_max}_step_{Num_inference_steps}_bs_{batch_size}_num_test_{num_test}_pgd_{pgd_step}_eot_{eot_step}.log")
...
edm_model = get_edm_cifar_uncond(pretrained=True, ckpt_dir=ckpt_dir).to(device)
edm_model.eval()
...
for sample_image, y_val in tqdm(cifar10_test_loader, colour='yellow'):
    ...
    for _ in tqdm(range(pgd_step), colour='red'):
        eot = 0
        for _ in range(1):
            images_1 = edm.purify_lock(x = sample_image + delta,sigma_max=sigma_max,num_steps=Num_inference_steps,noise_global=noise_global, reverse_noise=reverse_noise, S_churn=FLAGS.random_strength, S_min=FLAGS.s_min, S_max=FLAGS.s_max, S_noise=FLAGS.s_noise)
    ...
    with torch.no_grad():
        images_1 = edm.purify_lock(x = adv_out,sigma_max=sigma_max,num_steps=Num_inference_steps,noise_global=noise_global, reverse_noise=reverse_noise, S_churn=FLAGS.random_strength, S_min=FLAGS.s_min, S_max=FLAGS.s_max, S_noise=FLAGS.s_noise)
    with torch.no_grad():
        if FLAGS.model_type == "vit":

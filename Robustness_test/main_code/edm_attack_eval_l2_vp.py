...
logger.add(f"../../edm_result_attack/{FLAGS.save_folder}/{dir_temp}/edm_vp_l2_attack/{FLAGS.model_type}/log_test_edm_{dir_temp}_random_strength_{FLAGS.random_strength}_sigma_max_{sigma_max}_step_{Num_inference_steps}_bs_{batch_size}_num_test_{num_test}_pgd_{pgd_step}_eot_{eot_step}.log")
...
for sample_image, y_val in tqdm(cifar10_test_loader, colour='yellow'):
    ...
    for _ in tqdm(range(pgd_step), colour='red'):
        ...
        for _ in range(eot_step):
            ...
            #tmp_in = transform_cifar10((sample_image + delta)/2+0.5)
            if FLAGS.model_type == "vit":

...
logger.add(f"../vp_clean/{FLAGS.model_type}/log_test_fix_{FLAGS.fix_type}_ddpmpp_vp_t_{Denoise_strength}_step_{Num_inference_steps}_|{FLAGS.model_id.split('/')[-1]}|_bs_{batch_size}_num_test_{num_test}.log")
...
class ScoreSdeVpPipeline_Warp(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        for t in timesteps_list:
            ...
            with torch.no_grad():
                result = self.model(sample_image, scaled_t)
...
class ScoreSdeVpScheduler_Warp(ScoreSdeVpScheduler):  
    ...
    def step_correct(
        ...
        alpha_t = self.alphas_cumprod[scaled_t.long()]
...
for sample_image, y_val in tqdm(cifar10_test_loader, colour='yellow'):
    ...
    del images_1, yy, noisy_image, adv_out, noise, sample_image, y_val

...
class DDPMScheduler_Warp(DDPMScheduler):
    def add_noise(
        self,
...
def main(args):
    ...
    if args.model_config_name_or_path is None:
        ...
        model = UNet2DModel.from_pretrained(args.model_config_name_or_path, subfolder='unet')
    ...
    accepts_prediction_type = "prediction_type" in set(inspect.signature(DDPMScheduler_Warp.__init__).parameters.keys())
    if accepts_prediction_type:
        noise_scheduler = DDPMScheduler_Warp(
            num_train_timesteps=args.ddpm_num_steps,
            beta_schedule=args.ddpm_beta_schedule,
            prediction_type=args.prediction_type,
        )
    else:
        noise_scheduler = DDPMScheduler_Warp(num_train_timesteps=args.ddpm_num_steps, beta_schedule=args.ddpm_beta_schedule)
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            sqrt_alpha_prod, sqrt_one_minus_alpha_prod = noise_scheduler.get_ratio(timesteps)
            advnoise_ratio = torch.clamp(0.03 * sqrt_alpha_prod/sqrt_one_minus_alpha_prod, 0, 0.3)[:, None, None, None]
            ...
            for _ in range(5):
                for _ in range(1):
                    ...
                    noisy_images, sqrt_alpha_prod, sqrt_one_minus_alpha_prod = noise_scheduler.add_noise(clean_images, noise * (1 - advnoise_ratio ** 2) ** 0.5 + delta * advnoise_ratio, timesteps)
                    
                    noise_pred = model(noisy_images, timesteps, return_dict=False)[0]
                    original_samples = (noisy_images - sqrt_one_minus_alpha_prod * noise_pred)/sqrt_alpha_prod
                    ...
                    with torch.no_grad():
                        ...
                        delta = psedue_noise.reshape(*clean_images.shape).detach()
            ...
            with accelerator.accumulate(model):
                ...
                if args.prediction_type == "epsilon":
                    loss = F.mse_loss(model_output, noise * (1 - advnoise_ratio ** 2) ** 0.5 + delta * advnoise_ratio)  # this could have different weights!
